...
public class ResourceResolver implements LSResourceResolver {
	...
	/**
	 * <p>Constructor for ResourceResolver.</p>
	 *
	 * @param prefix a {@link java.lang.String} object.
	 */
	public ResourceResolver(String prefix) {
		log.debug("Setting up resolver, using URL prefix " + prefix);
		this.prefix = prefix;
	...
	public LSInput resolveResource(String type, String namespaceURI, String publicId, String systemId, String baseURI) {
		...
		if (log.isTraceEnabled())
			log.trace(systemId + " ### " + baseURI);

		String newSysId = prefix + systemId;

		if (systemId.equals("http://www.w3.org/2001/xml.xsd"))
			newSysId = prefix + "ext/xml.xsd";

		InputStream resourceAsStream;
		try {
			resourceAsStream = new URL(newSysId).openStream();
		...
		public String getStringData() {
			...
			synchronized (inputStream) {
				try {
					...
					log.error("When resolving " + systemId);
					e.printStackTrace();
					return null;
				}
			}
